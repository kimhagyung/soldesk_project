...
<!--모달-->
<!--모달-->
<div id="customModal" title="일정추가" style="display: none; width: auto; max-width: 100%;">
    <form:form action="${root}/mypage/calendar_pro" method="post" id="calendarForm">
        <!-- 일정 내용 입력란 -->
        <label for="eventTitle" style="display: block; margin-bottom: 5px; font-size: 18px;">일정을 추가하세요</label>
	...
	jq(function () {
        ...
        var calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            events: [
                ...
                {
                    title: "<c:out value="${event.calendar_memo}" />",
                    start: "<c:out value="${event.start_date}" />",
                    end: "<c:out value="${event.end_date}" />",
                    // 다른 이벤트 속성
                },
            	</c:forEach>
            ]
        ...
        function openCustomModal(start, end, allDay, title, event, memo, arg) {
		    ...
		    jq("#customModal").dialog({
		        ...
		        buttons: {
		            "저장": function () {
		                ...
		                sendSelectedDateToServer(start);  // start를 전달
        ...
        function sendSelectedDateToServer(selectedDate) {
            ...
            console.log("Sending selected date and memo:", formattedDate, memo);
            console.log("Memo value:", memo);

            jq.ajax({
                type: "POST",
                url: "${root}/mypage/calendar_pro",
                data: {
                    start_date: formattedDate,
                    calendar_memo: memo  // 메모 정보를 추가
                },
                success: function (response) {
                    console.log("Server response:", response);
                    console.log("Selected date and memo sent successfully:", response);
                },
                error: function (error) {
                    console.error("Error sending selected date and memo:", error);
                }
            });
        }
